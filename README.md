...
افزون یک برنامهٔ دو‌زبانه برای LuCI است که حافظه‌های USB، سواپ پایدار و extroot را در OpenWrt مدیریت می‌کند. زبان پیش‌فرض رابط انگلیسی است و از بالای صفحه می‌توان آن را به فارسی تغییر داد.

> [!WARNING]
> با زدن دکمهٔ **اعمال تغییرات**، جدول پارتیشن و تمام اطلاعات فلش انتخاب‌شده پاک می‌شود. قبل از ادامه از اطلاعات مهم نسخهٔ پشتیبان بگیرید. فلشی را که swap یا extroot فعال دارد ناگهانی جدا نکنید.

### امکانات

- تشخیص خودکار اتصال و جداسازی حافظهٔ USB
- نمایش ظرفیت، برند و مدل، پارتیشن‌ها، فایل‌سیستم و سرعت واقعی ارتباط USB
- امکان ساخت مستقل swap، extroot یا هر دو
- تعیین اندازه بر حسب MiB با فیلد عددی و پیشنهاد خودکار فضای باقی‌مانده برای extroot
...
```sh
opkg update
opkg install /tmp/luci-app-afzoon_1.0.0-r1_all.ipk
...
```sh
apk update
apk add --allow-untrusted /tmp/luci-app-afzoon-1.0.0-r1.apk
/etc/init.d/rpcd restart
/etc/init.d/uhttpd restart
```
...
4. گزینهٔ extroot را غیرفعال بگذارید.
...
1. فلش صحیح را انتخاب کنید.
2. گزینهٔ **Create persistent extroot** را فعال کنید.
...
extroot تا پیش از ریبوت فعال نمی‌شود.

### ساخت هم‌زمان swap و extroot

1. هر دو گزینه را فعال کنید.
2. اندازهٔ swap را وارد کنید.
3. برنامه فضای باقی‌مانده را پس از کنارگذاشتن حاشیهٔ ایمنی برای extroot پیشنهاد می‌دهد.
4. در صورت نیاز اندازهٔ extroot را تغییر دهید.
...
اگر extroot فعال است:

1. دکمهٔ **Disable extroot (reboot required)** را بزنید.
...
قبل از uninstall باید مطمئن شوید OpenWrt از extroot برنامه در حال اجرا نیست.

1. اگر extroot فعال است، آن را از داخل Afzoon غیرفعال کنید و در حالی که فلش متصل است روتر را ریبوت کنید.
...
- اگر دستگاه busy است، shareها و برنامه‌های استفاده‌کننده از فلش را ببندید. extroot فعال ابتدا باید غیرفعال و روتر ریبوت شود.
- اگر extroot پس از بوت فعال نشد، فلش را جدا یا دوباره پارتیشن‌بندی نکنید و این فرمان‌ها را بررسی کنید:
...
ساخته‌شدن موفق بسته به‌تنهایی اثبات نمی‌کند که extroot روی تمام روترها کار می‌کند؛ پشتیبانی نهایی به سخت‌افزار، kernel، توان USB و firmware همان روتر وابسته است.
